...
# print header
print ("#SPECIES_TREE:", pearsonr(STdists, trDist)[0], pearsonr(STdists, trDist)[1])
...
#for PGLS uncomment
myCorr = pearsonr(GTdists, trDist)
for i in range(len(STdists)):
        vals[(spec[i][0],spec[i][1])][0] = [GTdists[i], trDist[i]]
...
for k in range(1,numSims+1):
    ...
    #for PGLS uncomment
    for i in range(len(STdists)):
        vals[(spec[i][0],spec[i][1])][k] = [GTdists[i]]
k = -1
old = ''
for specPair in vals:
    new = specPair[0]
    if new != old:
        old = new
        k += 1
    print (str(specPair[0])[1:-1],str(specPair[1])[1:-1]+'-'+str(k),k,end=' ') 
    for i in vals[specPair]:
        for tr in vals[specPair][i]:
            print (tr, end= ' ')
    print()
