...
readAndDoPGLS<-function(n,i) {
    ...
    for(j in seq(2,2)) { # switch to seq(0,7) for all species -- 2 is P. commune
        ...
        for (c in seq(6,length(colnames(locVals))-1)) {
          ...
          modNaive<-lm(regVals$tr ~ regVals$gd)
          myCoef <- try(coef(summary(mod))[,4][2])
          if (is.numeric(myCoef)) {
              nullmods <- c(nullmods,myCoef)
              nullmodsNaive <- c(nullmodsNaive,summary(modNaive)$coefficients[8])
          }
        }
        
    }
    tm <- data.frame(p=truemods,r=truemodsNaive,val="true")
    nm <- data.frame(p=nullmods,r=nullmodsNaive,val="null")
    #tm <- rbind(tm, data.frame(p=truemodsNaive, val="trueNaive"))
    #nm <- rbind(nm, data.frame(p=nullmodsNaive, val="nullNaive"))
